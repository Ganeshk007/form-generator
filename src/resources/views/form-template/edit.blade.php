...
<body>
    ...
    <div class="container">
        <h2 class="mb-0">Edit Form Template
        ...
        <input type="text" id="template_name" name="template_name" value="{{ $template->name }}" required>

        <!-- Stub for the template -->
        <label for="template_stub">Template Stub:</label>
        <textarea id="template_stub" name="template_stub" rows="4">{{ $template->stub }}</textarea>
        ...
        <div id="fields-container" class="fields-container">
            @foreach($template->fields as $field)
                <div class="field-group">
                    ...
                    <select class="field-name" onchange="checkDuplicate(this)">
                        @foreach(config('form-generator.input_types') as $option)
                            <option value="{{ $option['type'] }}" {{ $option['type'] == $field['type'] ? 'selected' : '' }}>
                                {{ $option['name'] }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <textarea class="field-stub">{{ $field['stub'] }}</textarea>
        ...
        <!-- Update Button -->
        <button type="button" class="btn" onclick="updateTemplate()">Update Template</button>
    </div>
    ...
    <script>
        ...
        function updateTemplate() {
            ...
            $.ajax({
                url: "{{ route('form-generator.template.update', ':id') }}".replace(':id', templateId),
                type: "PUT",
                contentType: "application/json",
                data: JSON.stringify(formData),
                success: function (response) {
                    alert("Template updated successfully!");
                    window.location.href = "{{ route('form-generator.template') }}";
                },
                error: function (err) {
